...
public enum AnnoColor {
    /// Палитра из 8 контрастных цветов. Порядок обязан совпадать с web `PALETTE`,
    /// иначе цвет оператора разъедется между платформами.
    ///
    /// Лимита на число агентов в сессии больше нет, так что при большом co-viewing
    /// цвета повторяются. Здесь это заметнее, чем на web: подписей автора в
    /// рендерере нет, цвет — единственный признак, кто рисует.
