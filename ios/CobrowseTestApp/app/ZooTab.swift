...
//  типографика мелкого/крупного текста. Плюс вход в «глубокие» настройки
//  (Настройки → Помощь и поддержка → Чат с поддержкой).
//
...
struct ZooTab: View {
    ...
    var body: some View {
        NavigationView {
            List {
                // Точка входа в «глубокие» настройки → Помощь → Чат с поддержкой.
                // Сверху, а не после длинного списка, чтобы находить без скролла.
                Section("Приложение") {
                    NavigationLink {
                        SettingsView()
                    } label: {
