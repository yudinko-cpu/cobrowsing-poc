...
/**
 * Палитра из 8 контрастных цветов. Порядок обязан совпадать со Swift
 * `AnnoColor.palette`, иначе цвет оператора разъедется между web и iOS —
 * поэтому менять её можно только синхронно с обеими сторонами.
 *
 * Раньше размер палитры был равен лимиту агентов в сессии; лимита больше нет,
 * поэтому при большом co-viewing цвета начнут повторяться. Деградация мягкая
 * (коллизия цвета, не падение), но на iOS цвет — единственный признак автора:
 * подписей там нет. Если демо регулярно собирает больше ~8 операторов,
 * расширять надо палитру, а не этот комментарий.
 */
