...
- **Мульти-оператор.** Несколько операторов (co-viewing) рисуют одновременно,
  аннотации атрибутированы по автору и разноцветны. Требование заложено в
  протокол и модель хранения с первого дня. Лимита на число операторов в backend
  нет; палитра при этом на 8 цветов, поэтому дальше цвета начнут повторяться.
...
```jsonc
{
  ...
  "op": "add|append|end|remove|clear|pointer|click|chat|typing|chat-sync|sync-req|sync-state",
...
| `text` | `color`, `size`, `at:[nx,ny]`, `text` | `add` при постановке → `append` при редактировании текста → `end`. *Тулбар больше не создаёт текст — его заменил чат поддержки (см. «Чат» ниже); тип сохранён в протоколе и рендерерах для совместимости.* |
...
| `click` (клик указкой) | `color`, `at:[nx,ny]` | отдельный `op:"click"`, **reliable** (разовое событие-акцент). Эфемерная «волна»: кольцо расходится и гаснет за 600 мс. Ключ в сторе — `author:ts`, поэтому серия кликов сосуществует |
...
- Поля: `id` (`author:counter`), `text` (≤ `MAX_CHAT_LEN` = 1000 символов, trim
  на обеих сторонах). Reliable, broadcast всем в комнате — коллеги-операторы
  видят переписку.
- Двунаправленный: единственный op, который вправе слать клиент (web пропускает
  его до гейта прав §6.4). `author` на приёме перезаписывается аутентифицированной
  identity отправителя; дедуп по ключу `identity|id`.
- Не является состоянием аннотаций и не входит в `sync-state`. История сессии
  живёт в памяти телефона (`ChatStore` — канонический стор, как у аннотаций) и
  никуда не пишется; на `sync-req` телефон вместе с `sync-state` шлёт
  запросившему `chat-sync {history:[{id, author, text, ts}, …]}` пакетами
  ≤ ~12 КБ строго по порядку, так что поздно подключившийся или обновивший
  вкладку оператор видит всю переписку. Элементы истории несут исходные
  `id`/`author`/`ts`; свои сообщения телефон помечает псевдо-автором `client`,
  web подставляет под него identity телефона, поэтому ключи дедупа `identity|id`
  совпадают с живыми. `chat-sync` принимается только от Customer.
- UX на клиенте: во время сессии виден перетаскиваемый полупрозрачный FAB с
  ...
  Всякое → Настройки → Помощь и поддержка → Чат с поддержкой). Рядом с FAB —
  стопка превью последних входящих (до 3, каждое ≤ 3 строк), чтобы читать
  ответы оператора, не открывая чат; тап «лопает» пузырёк. Превью — уведомления,
  а не «прочитано»: бейдж гасит только открытие чата (как баннеры iOS и превью
  Intercom) — превью обрезано, «увидел» ≠ «прочитал».
...
- **reliable = true:** `add`(старт фигуры), `click`, `chat`, `end`, `remove`,
  `clear`, `sync-state` — критично не потерять.
...
4. Тем же ответом клиент шлёт историю чата сессии — `chat-sync` (§5), тоже
   адресно и пакетами; панель чата мёржит её по ключу `identity|id`.
...
- **Только операторы рисуют.** На приёме (и на iOS, и на web) ops от участника с
  `name == 'Customer'` игнорируются — кроме `chat` (§5): это единственное, что
  клиент вправе слать. Аннотаций клиент никогда не автор.
...
- **Текст (text).** *Из тулбара убран — заменён чатом поддержки (§5, `op: "chat"`):
  надпись в произвольной точке делала экран клиента похожим на холст.* Тип
  сохранён в протоколе и рендерерах: текст едет как строка, рендерится выбранным
  цветом и норм. кеглем, длина ограничена (200 символов), содержимое санитизируется.
- **Лазерная указка (pointer).** Эфемерная точка, следует за курсором оператора в
  ...
  **Клик указкой** (нажатие мышью в режиме указки) шлёт отдельный reliable
  `click` — в точке нажатия у всех расходится кольцо (600 мс). Это визуальный
  аналог «тапа»: показывает не только «смотри сюда», но и «нажми вот здесь».
...
- **SVG-overlay** (векторный, чёткий текст, простой хит-тест) абсолютно
  ...
  dims трека.
- **Тулбар:** выбор инструмента (рисование / стрелка / прямоугольник / овал /
  указка / undo / clear). Цвет — авто по своей identity (свой цвет оператора).
- **Чат с клиентом:** правая колонка страницы сессии (`ChatPanel`) — свой
  слушатель `DataReceived` на том же топике с фильтром `op === 'chat'`,
  отправка тем же `publishData` (reliable).
...
| 8 | **Права** | — | У клиента нет UI рисования; ops от клиента отбрасываются (кроме `chat` — он показывается в панели чата), `sync-state` принимается только от него |
