...
- **Политика `clear all`.** Кто вправе стереть чужие аннотации при нескольких
  ...
  прод-RBAC.
- **Размер/частота data.** Указка и стриминг штрихов высокочастотны — держим lossy
  и батчинг, следим, чтобы data-канал не конкурировал с медиа (ANNO-7).
...
- **Спуфинг автора.** Любой участник с `canPublishData` технически может слать ops;
  защищаемся ролью (`name=='Customer'` игнорируем) — но это app-level, не крипто.
  Достаточно для PoC.
