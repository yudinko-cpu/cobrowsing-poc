...
- Не является состоянием аннотаций и не входит в `sync-state`: истории нет, всё
  живёт в памяти одной сессии (iOS `ChatStore`, web `ChatPanel`), поздно
  подключившийся оператор видит только новые сообщения.
